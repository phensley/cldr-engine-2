...
export interface DeriveOptions {
  /** Locale universe override (the FULL published set); default = the committed packs dir. */
  universe?: readonly string[];
}

export const deriveManifest = (opts: DeriveOptions = {}): DerivedManifest => {
  ...
  const locales = opts.universe !== undefined ? [...opts.universe].sort() : localeStems();

  return { locales, features, info };
};

/** Committed per-locale pack stems → locale tags. */
const localeStems = (): string[] =>
...
export const renderManifest = (d: DerivedManifest): string => [RENDER_HEADER, TYPES, renderManifestConst(d, { typed: true })].join('\n');
...
/** The manifest VALUE module (JS body — types stripped) for the full-universe dist build. */
export const renderManifestConst = (d: DerivedManifest, opts: { typed?: boolean } = {}): string => {
...
${renderSlots(f.slots, '        ')}      },
    ...
    .join('\n');
  const annotation = opts.typed ? ': Manifest' : '';
  return `export const manifest${annotation} = {\n  locales: [${d.locales.map((l) => `'${l}'`).join(', ')}],\n  features: {\n${features}\n  },\n};\n`;
};

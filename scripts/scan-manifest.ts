...
const PKG_PATH = join(ROOT, 'packages', 'cldr', 'package.json');
const PKG = JSON.parse(readFileSync(PKG_PATH, 'utf8')) as { exports: Record<string, { types: string; default: string }> };
...
const verifyRefs = (feature: string, slots: Record<string, FeatureSlotLike>): void => {
  ...
  const exported = PKG.exports[`./${feature}/*`];
  if (exported === undefined) {
    throw new Error(`package.json exports missing './${feature}/*' — subpaths like @phensley/cldr/${feature}/… would be unpublishable`);
  }
...
${renderSlots(f.slots, '        ')}      },
  ...
  return [
    ...
    ' * the impl modules (each ref verified to exist), factory.ts, package.json exports,',
    ' * and the packs/ directory (locales). Per-feature pack-stream metadata is declared',
...
if (isMain()) {
  ...
  console.log('scan-manifest: all slot refs verified (module + export + exports map)');
